...
public class TimeScheduler {
    ...
    public void chatAlertScheduler() {
        ...
        List<String> lastMessageTimeList = redisRepository.getLastMessageReadTime();
        for (String time : lastMessageTimeList){
            LocalDateTime lastMessageTime = LocalDateTime.parse(time, formatter);

            Long timeDiff = Duration.between(lastMessageTime, currentTime).getSeconds();
            if ((timeDiff / 3600) > 0) { // 시간

                //messageService.createChatRoomAlert();
            }
